...
**핵심 규칙**: Actions → Service → Repository 단방향 호출만 허용. Actions에서 DB 직접 접근 금지. Repository에서 비즈니스 로직 금지.
...
type GetProductInput = { id: number }
...
export const ProductService = {
  findById: async (id: number): Promise<Product> => {
    ...
    if (!product) throw new AppError(ErrorCode.NOT_FOUND, '상품을 찾을 수 없습니다')
    return product
  },
...
**규칙**: `export const ServiceName = { ... }` 형태. Repository만 호출. 데이터 없음 → `AppError` throw (null 반환 금지). R2·sharp 직접 호출 가능.
...
export const ProductRepository = {
  ...
  update: async (id: number, data: Partial<Omit<Product, 'id' | 'createdAt'>>): Promise<Product | null> => {
    const result: Product[] = await db.update(products).set({ ...data, updatedAt: new Date() }).where(eq(products.id, id)).returning()
    return result[0] ?? null
  },
...
// user.id를 Service에 전달
```

**규칙**: 인증 확인은 입력 검증 직후, Service 호출 전. 비인증 시 즉시 `UNAUTHORIZED` 반환.
...
create: async (input: CreateProductInput): Promise<Product> => {
  ...
  return ProductRepository.create({ ...input, thumbnailUrl, detailImageUrl, isActive: true })
},

// Actions에서 FormData → Buffer 변환 후 Service 전달
const file = formData.get('thumbnail') as File | null
if (!file) return { success: false, error: '이미지가 필요합니다', code: 'VALIDATION_ERROR' }
const thumbnailBuffer: Buffer = Buffer.from(await file.arrayBuffer())
```

**규칙**: R2·sharp 호출은 Service에서만. 이미지 업로드는 DB 저장 전 수행.
...
npx drizzle-kit push       # 개발 DB에 적용
```

**규칙**: 스키마 변경 → generate → push 순서 준수. `push`는 개발/테스트 환경에서만. `src/db/migrations/`는 git에 커밋.
